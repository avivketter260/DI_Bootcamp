...
loop through the array of
 object and return the first car with the name “Honda*.
then, return a string in the format This is a {car_make} {car_model} from {car_year}.
Hint : Find an array method that returns the value of 
the first element in an array that pass a test.
...
//         if (i.car_make === 'Honda') return console.log(`his is a ${i.car_make} ${i.car_model} from ${i.car_year}`)
//     }
// }
...
const sortCarInventoryByYear = (carInventory) => {
    ...
    for (let i = dates.length - 1; i >= 0; --i) {
        for (let j of carInventory) {
            if (j.car_year === dates[i]) {
                    result.push(j)
                    dates.pop()
                } else continue;
        }
